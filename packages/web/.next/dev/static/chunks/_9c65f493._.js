...
(()=>{
    const e = new Error("Cannot find module '@/lib/similarity'");
...
function ExplorePage() {
    ...
    const handleSearch = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useCallback"])({
        "ExplorePage.useCallback[handleSearch]": async (query)=>{
            ...
            const { data: profiles } = await supabase.from("profiles").select("id, username, avatar_url").ilike("username", `%${query}%`).limit(20);
            if (!profiles?.length) {
                setResults([]);
                return;
            }
            ...
            if (user) {
                ...
                if (viewerList) {
                    const { data } = await supabase.from("list_items").select("show_id, rating").eq("list_id", viewerList.id);
                    viewerItems = data ?? [];
                }
            }
            const viewerListData = viewerItems.map({
                "ExplorePage.useCallback[handleSearch].viewerListData": (i)=>({
                        showId: i.show_id,
                        rating: i.rating
                    })
            ...
            for (const p of profiles){
                ...
                const { data: listItems, count } = await supabase.from("list_items").select("show_id, rating", {
                    count: "exact"
                }).eq("list_id", userList.id);
                let similarity = null;
                if (user && viewerListData.length > 0 && listItems && listItems.length > 0) {
                    const otherListData = listItems.map({
                        "ExplorePage.useCallback[handleSearch].otherListData": (i)=>({
                                showId: i.show_id,
                                rating: i.rating
                            })
...
_s(ExplorePage, "mG6F29QQs+ZiIE2NrHmH8xcnLq4=", false, function() {
    return [
...
//# sourceMappingURL=_9c65f493._.js.map