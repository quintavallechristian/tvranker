...
import type { AnimePodiumItem } from "@/app/[locale]/(app)/home/actions";
import { Link } from "@/i18n/navigation";
import { useTranslations } from "next-intl";

export function AnimePodiumWidget({
  items,
  rowSpan = 1,
  viewAllHref,
  badge,
}: {
  items: AnimePodiumItem[];
  rowSpan?: 1 | 2;
  viewAllHref?: string;
    ...
    <div className="flex h-full flex-col overflow-hidden rounded-xl border border-border bg-bg-surface p-4">
      <div className="mb-3 flex shrink-0 items-center justify-between">
        <p className="text-xs font-medium uppercase tracking-widest text-text-muted">
          {rowSpan === 2 ? t("widgets.top10Anime") : t("widgets.animePodium")}
        </p>
        {badge ?? (
          <Link
            href={viewAllHref ?? "/anime"}
      ...
      {items.length > 0 ? (
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-border py-6">
          ...
          <p className="text-xs text-text-muted">{t("emptyAnimeList")}</p>
        </div>
      )}
    </div>
  );
...
function RankRow({ item, rank }: { item: AnimePodiumItem; rank: number }) {
  const posterUrl = getPosterUrl(item.poster_path, "w92");
