...
export default function ExploreShowsPage() {
  ...
  const handleSearch = useCallback(async (query: string) => {
    ...
    setSearched(true);
    const data = await fetch(`/api/tmdb/search?q=${encodeURIComponent(query)}`).then((r) => r.json());
    setShowResults(
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-1">
        ...
        <h1 className="text-xl font-bold text-text-primary">{t("showsTitle")}</h1>
            ...
            >
              ...
              >
                <div className="relative h-12 w-8 shrink-0 overflow-hidden rounded-sm bg-bg-elevated">
                  {show.poster_path ? (
                    <Image src={getPosterUrl(show.poster_path, "w92")!} alt={show.title} fill className="object-cover" sizes="32px" />
                  ) : (
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {show.first_air_date && (
                    <p className="text-xs text-text-muted">{show.first_air_date.slice(0, 4)}</p>
                  )}
                </div>
              ...
              >
                {addingTmdbId === show.tmdb_id ? <SpinnerGap size={14} className="animate-spin" /> : addedTmdbIds.has(show.tmdb_id) ? <Check size={14} weight="bold" /> : <Plus size={14} weight="bold" />}
              </button>
            </div>
          ))}
        </div>
              ...
              {recommendations.map((show) => {
                ...
                return (
                  <div
                    ...
                    onPointerEnter={(e) => { if (e.pointerType === "mouse") setActiveShowId(show.id); }}
                    onPointerLeave={(e) => { if (e.pointerType === "mouse") setActiveShowId(null); }}
                    onClick={(e) => { e.stopPropagation(); setActiveShowId(show.id); }}
                  >
                    ...
                    <div className="relative aspect-2/3 w-full bg-bg-elevated">
                      {posterUrl ? (
                        <Image src={posterUrl} alt={show.title} fill className="object-cover" sizes="(max-width: 640px) 50vw, (max-width: 768px) 33vw, 20vw" />
                      ) : (
                        <div className="flex h-full items-center justify-center">
                          <Television size={32} className="text-text-faint" />
                        </div>
                      )}
                      <div className={`absolute inset-0 flex flex-col items-center justify-center gap-2.5 bg-black/70 transition-opacity ${activeShowId === show.id || isAdded ? "opacity-100" : "pointer-events-none opacity-0"}`}
                        onClick={(e) => { e.stopPropagation(); setActiveShowId(null); }}>
                        {isAdded ? (
                          <Check size={24} weight="bold" className="text-accent" />
                        ) : (
                          <>
                            <button onClick={(e) => { e.stopPropagation(); handleAddToList(show); }} disabled={isAdding}
                              ...
                              {isAdding ? <SpinnerGap size={13} className="animate-spin" /> : <Plus size={13} weight="bold" />}
                              {t("addToList")}
                            </button>
                    ...
                    <div className="p-3">
                      ...
                      <p className="mt-0.5 text-xs text-text-muted">{t("recommendedBy", { count: show.recommendedBy })}</p>
                    </div>
                  </div>
                );
              })}
                ...
                {popularShows.map((show) => {
                  ...
                  return (
                    <div key={show.id}
                      ...
                      <div className="relative aspect-2/3 w-full bg-bg-elevated">
                        {posterUrl ? (
                          <Image src={posterUrl} alt={show.title} fill className="object-cover" sizes="(max-width: 640px) 50vw, 17vw" />
                        ) : (
                          <div className="flex h-full items-center justify-center">
                      ...
                      <div className="p-2">
                        ...
                        <p className="mt-0.5 text-[10px] text-text-faint">{t("addedByCount", { count: show.addedCount })}</p>
                      </div>
                    </div>
                  );
                })}
