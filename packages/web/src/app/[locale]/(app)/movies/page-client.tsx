...
export function MovieListClient({
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => {
      if (q && !item.movies.title.toLowerCase().includes(q)) return false;
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      startTransition(async () => {
        await reorderMovieListItems(newSorted);
      });
    },
  ...
  const handleDescriptionChange = useCallback(
    (value: string) => {
      ...
      descDebounceRef.current = setTimeout(() => {
        startTransition(async () => {
          await updateMovieListDescription(movieListId, value);
          setSaveStatus("saved");
          savedTimerRef.current = setTimeout(() => setSaveStatus("idle"), 2000);
        });
      }, 800);
    },
    [movieListId, startTransition],
  );
  ...
  const handleAdd = useCallback(
    async (movie: {
      ...
      startTransition(async () => {
        await addMovieToList(movieListId, movie);
        router.refresh();
      });
      setExistingTmdbIds((prev) => [...prev, movie.tmdb_id]);
    },
    [movieListId, router, startTransition],
  );
  ...
  const handleRemove = useCallback(
    (itemId: string) => {
      ...
      startTransition(async () => {
        await removeMovieFromList(itemId);
      });
    },
    [startTransition],
  );
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        ...
        {/* Mobile-only: Add movie + import buttons below title */}
        {items.length > 0 && (
                      ...
                      {group.items.map((item) => {
                        ...
                        return (
                          <ShowRow
                            ...
                            posterPath={item.movies.poster_path}
                            rating={item.rating}
                            position={globalIndex + 1}
                            ratingLabels={ratingLabels}
                            detailHref={`/movies/${item.movie_id}`}
      ...
      {listSettings && profileVisibility && (
        <ListSettingsModal
          ...
          onSave={async (id, updates) => {
            await updateMovieListSettings(id, updates);
          }}
        />
      )}
    </div>
